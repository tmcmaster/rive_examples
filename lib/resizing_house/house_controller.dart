...
class HouseController extends FlareController {
  ...
  bool advance(FlutterActorArtboard artboard, double elapsed) {
    ...
    double demoMix = _demoAnimation.mix + DemoMixSpeed * (isDemoMode ? elapsed : -elapsed);
    demoMix = demoMix.clamp(0.0, 1.0);
    _demoAnimation.mix = demoMix;

    if (demoMix != 0.0) {
      /// Advance the time, and loop.
      _demoAnimation.time = (_demoAnimation.time + elapsed) % _demoAnimation.duration;
  ...
  void initialize(FlutterActorArtboard artboard) {
    _artboard = artboard;
    final demoMode = _artboard.getAnimation('Demo Mode');
    final sunRotate = _artboard.getAnimation('Sun Rotate');
    if (demoMode != null) _demoAnimation = FlareAnimationLayer('Demo Mode', demoMode);
    if (sunRotate != null) _skyAnimation = FlareAnimationLayer('Sun Rotate', sunRotate)..mix = 1.0;
    final endAnimation = artboard.getAnimation("to 6");
  ...
  _checkRoom() {
    ...
    if (demoFrame <= 15) {
      demoValue = lerpDouble(6.0, 5.0, Curves.easeInOut.transform(demoFrame / 15));
    } else if (demoFrame <= 36) {
      demoValue = 5.0;
    } else if (demoFrame <= 50) {
      demoValue = lerpDouble(5.0, 4.0, Curves.easeInOut.transform((demoFrame - 36) / (50 - 36)));
    } else if (demoFrame <= 72) {
      demoValue = 4.0;
    } else if (demoFrame <= 87) {
      demoValue = lerpDouble(4.0, 3.0, Curves.easeInOut.transform((demoFrame - 72) / (87 - 72)));
    } else if (demoFrame <= 128) {
      demoValue = 3.0;
    } else if (demoFrame <= 142) {
      demoValue = lerpDouble(3.0, 6.0, Curves.easeInOut.transform((demoFrame - 128) / (142 - 128)));
